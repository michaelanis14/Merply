...
void RptCrossTabObject::makeFeelMatrix() {
	qDebug() << __FILE__ << __LINE__  << "makeFeelMatrix";
	...
	qDebug() << __FILE__ << __LINE__  << "AllCol:" << allColCount() << "Col:" << colCount();
	qDebug() << __FILE__ << __LINE__  << "AllRow:" << allRowCount() << "Row:" << rowCount();
...
void RptCrossTabObject::addElement(RptTabElement element) {
	...
	for (int row=0; row < rowVector.size(); row++) {
		if (element.top <= rowVector.at(row)+correlation &&
			...
			fnd = true;
			//qDebug() << __FILE__ << __LINE__ << element.top << rowVector.at(row)+correlation << rowVector.at(row)-correlation;
